...
namespace Network.Core
{
    public class GameClient_Name
    {
        ...
        private readonly ProtobufDispatcher_Name m_dispatcher = new();
        ...
        public void StartTcp(string serverIP,int serverTcpPort, int serverUdpPort = 0)
        {
        ...
        {   
            var pkgHead = new MessageHeader_Name(body);
            var data = new Buffer(pkgHead.FullLength + 4);
        ...
        private async Task TcpRecvThread(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested && tcp_session.IsStateConnected())
            {
                ...
                try
                {
                    ...
                    while (recvBuf.DataSize >= MessageHeader_Name.kMinHeaderLen)
                    {
                        (MessageHeader_Name header, IMessage msg) = ParseTcp(recvBuf, out MessageParseErrorCode errCode);
        ...
        private async Task UdpRecvThread(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    while (recvBuf.DataSize >= MessageHeader_Name.kMinHeaderLen)
                    {
                        (MessageHeader_Name header, IMessage msg) = ParseUdp(recvBuf, out MessageParseErrorCode errCode);
                        switch (errCode)
                        {
                            case MessageParseErrorCode.eNoError:
                                if (msg != null) {
        ...
        private (MessageHeader_Name, IMessage) ParseTcp(Buffer recvBuf, out MessageParseErrorCode outErrCode)
        {
            //! 解析消息头，获得消息体长和RPC名称
            MessageHeader_Name header = default;
            outErrCode = header.RetrieveFromBuffer(recvBuf, tcp_session.xor_code);
            if (outErrCode != MessageParseErrorCode.eNoError) 
                return (header, null);
            
            //! 解析消息体
            IMessage message = m_dispatcher.TryCreateMessage(header.Typename);
            if (message != null) {
                //! 消费缓冲区
        ...
        private (MessageHeader_Name, IMessage) ParseUdp(Buffer recvBuf, out MessageParseErrorCode outErrCode)
        {
            //! 解析消息头，获得消息体长和RPC名称
            MessageHeader_Name header = default;
        ...
        public void DisconnectServer(string errmsg)
        {
            if (!tcp_session.IsStateConnected()) return;
            Debug.LogWarning($"DisconnectServer: {errmsg}");
            lock (this)
            {
                m_ctsTcp.Cancel();
                m_ctsUDP.Cancel();
        ...
        private void OnSecurityResult(SecurityCheckRsp result)
        {
            ...
            if (result.ResultCode != SecurityCheckRsp.Types.ResultCode.ESuccess) {
                ...
                var localEp = (IPEndPoint)udp_session.sock.Client.LocalEndPoint;
                UdpPortRegisterReq req = new()
                {
                    SessionId = this.SessionID,
                    ClientUdpIp = localEp.Address.ToString(),
                    ClientUdpPort = (uint)localEp.Port,
                };
                Debug.Log($"向服务器申请注册客户端Udp端口[{localEp}]");
